...
public class Inventario : Singleton<Inventario>
{
    ...
    private readonly string INVENTARIO_KEY = "aNd3sGu4rdi4Nes2420";
    ...
    private void CargarInventario()
    {
        ...
        {
            InventarioData dataCargando = SaveGame.Load<InventarioData>(INVENTARIO_KEY);
        for(int i = 0; i < numeroDeSlots; i++)
        {
            if(dataCargando.ItemsDatos[i] != null)
            {
                InventarioItem itemAlmacen = ItemExisteEnAlmacen(dataCargando.ItemsDatos[i]);
                if (itemAlmacen != null)
                {
                    itemsInventario[i] = itemAlmacen.CopiarItem();
                    itemsInventario[i].Cantidad = dataCargando.ItemsCantidad[i];
